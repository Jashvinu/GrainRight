...
const SEASONAL_THRESHOLDS: Record<
  Season,
  Record<string, { low: number; warning: number }>
> = {
  winter: {
...
function getSeasonalThresholds(): Record<
  string,
...
function calculateSpectralFeatures(image: any): Record<string, any> {
  ...
  const swirBalance = swir1.subtract(swir2).divide(swir1.add(swir2)).rename(
    "SWIR_Balance",
  );
  const bsi = swir1.add(red).subtract(nir.add(blue)).divide(
    swir1.add(red).add(nir).add(blue),
  ).rename("BSI");
  const savi = nir.subtract(red).multiply(1.5).divide(nir.add(red).add(0.5))
    .rename("SAVI");
...
async function generateAndUploadRaster(
  ...
  try {
    ...
    if (!imgResponse.ok) {
      console.warn(
        `[Diagnostics] Thumb fetch failed for ${index}: ${imgResponse.status}`,
      );
      return null;
    }
...
function isValidCoordinate(point: unknown): point is number[] {
...
function isClosedRing(ring: unknown): ring is number[][] {
  ...
  if (!ring.every(isValidCoordinate)) return false;
  const first = ring[0];
  const last = ring[ring.length - 1];
  return first[0] === last[0] && first[1] === last[1];
}

function validateGeoJsonPolygon(geometry: any): void {
  if (!geometry || typeof geometry !== "object") {
    throw new Error("polygon must be a GeoJSON Polygon or MultiPolygon object");
  }
  ...
  if (geometry.type === "Polygon") {
    if (
      ...
      throw new Error("Polygon coordinates must contain at least one ring");
    }
    if (!isClosedRing(geometry.coordinates[0])) {
      throw new Error(
        "Polygon outer ring must be closed [longitude, latitude] coordinates",
      );
    }
    return;
  }
  ...
  if (geometry.type === "MultiPolygon") {
    ...
    for (const polygon of geometry.coordinates) {
      if (
        ...
        !isClosedRing(polygon[0])
      ) {
        throw new Error(
          "Each MultiPolygon part must contain a closed outer ring",
        );
      }
    }
    return;
...
async function processIndex(
  ...
  const [statsResult, firstStatsResult, lastStatsResult, mapResult] =
    await Promise.all([
      (async () => {
        ...
        console.log(
          `[Diagnostics] ${index} stats: ${timings[`${index}_stats`]}ms`,
        );
        return stats;
      })(),
      ...
      (async () => {
        ...
        try {
          ...
          console.log(
            `[Diagnostics] ${index} first image: ${
              timings[`${index}_firstImage`]
            }ms`,
          );
          return stats;
        } catch (e) {
      ...
      (async () => {
        ...
        try {
          ...
          console.log(
            `[Diagnostics] ${index} last image: ${
              timings[`${index}_lastImage`]
            }ms`,
          );
          return stats;
        } catch (e) {
      ...
      (async () => {
        const start = Date.now();
        try {
          const mapData = await getMapIdWithRetry(
            indexImage.clip(eeGeometry),
            visParams,
            3,
            1000,
          );
          ...
          console.log(
            `[Diagnostics] ${index} map tile: ${timings[`${index}_mapTile`]}ms`,
          );
          return mapData;
        } catch (e) {
  ...
  console.log(
    `[Diagnostics] ${index} parallel ops: ${Date.now() - parallelOpsStart}ms`,
  );
  ...
  if (firstStatsResult && lastStatsResult) {
    ...
    if (trendUnit === "points") {
      ...
      trendDetected = trend < NUTRIENT_TREND_THRESHOLD_POINTS &&
        lastMean < threshold.warning;
    } else if (firstMean !== 0) {
  ...
  if (belowThreshold || trendDetected) {
    problem = {
      index,
      type: belowThreshold && trendDetected
        ? "both"
        : (belowThreshold ? "threshold" : "trend"),
      avgValue: mean,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!farmId) {
      return errorResponse("farm_id parameter is required", 400);
    }

    let geometry: any;
    try {
      geometry = JSON.parse(polygonParam);
      validateGeoJsonPolygon(geometry);
    } catch (e) {
      return errorResponse(
        e instanceof Error ? e.message : "Invalid polygon JSON",
        400,
      );
    }

    const indices = indicesParam.split(",").filter((i) =>
      DIAGNOSTIC_INDICES.includes(i)
    );
    if (indices.length === 0) {
      return errorResponse("At least one supported index is required", 400);
    }
    ...
    if (cached) {
      ...
      if (cacheHasCurrentModel) {
        ...
        return successResponse({
          ...
          cellData: cachedCellStats,
          cell_stats: cachedCellStats,
      ...
      console.log(
        "[Diagnostics] Cache stale for farm:",
        farmId,
        "— contract mismatch",
      );
    }
    console.log(
      "[Diagnostics] Cache miss for farm:",
      farmId,
    ...
    if (
      !serviceAccountKey.project_id || !serviceAccountKey.private_key ||
    ...
    console.log(
      `[Diagnostics] Season: ${season}, Processing indices: ${
        indices.join(", ")
      }`,
    );
    ...
    const startDate =
      new Date(Date.now() - numDays * 24 * 60 * 60 * 1000).toISOString().split(
        "T",
      )[0];
    console.log(
      `[Diagnostics] Date range: ${startDate} to ${endDate} (${numDays} days)`,
    );
    ...
    if (!imageCount || imageCount <= 0) {
      throw new Error(
        `No usable optical images found below ${maxCloudCover}% cloud cover for ${startDate} to ${endDate}`,
      );
    }

    const sortedCollection = collection.sort("system:time_start", false);
    console.log(
      `[Diagnostics] Collection setup: ${Date.now() - collectionStart}ms`,
    );
    ...
    console.log(
      `[Diagnostics] Pre-computed shared images: ${
        Date.now() - precomputeStart
      }ms`,
    );
    ...
    console.log(
      `[Diagnostics] Starting parallel processing of ${indices.length} indices...`,
    );

    const results = await Promise.all(
      indices.map((index) =>
        processIndex(index, precomputed, eeGeometry, timings)
      ),
    );

    console.log(
      `[Diagnostics] Parallel processing complete: ${
    ...
    const indexBands = indices.map((index) =>
      INDEX_CALCULATORS[index](precomputed.composite)
    );
    ...
    // Sample at 30m resolution. The client may propagate these real samples to
    // finer display cells, but the server must not synthesize source samples.
    ...
    if (features.length === 0) {
      throw new Error(
        "No diagnostic cell samples were returned for this farm polygon",
      );
    }
    console.log(
      `[Diagnostics] Sampled ${features.length} points in ${
    ...
    const rasterResults = await Promise.all(
      indices.map(async (index) => {
        const clippedImage = INDEX_CALCULATORS[index](precomputed.composite)
          .clip(eeGeometry);
        const publicUrl = await generateAndUploadRaster(
          index,
          clippedImage,
          farmId,
          timestamp,
        ...
        if (!publicUrl) {
          throw new Error(`Raster generation/upload failed for ${index}`);
        }
    ...
    rasterResults.forEach(({ index, publicUrl }) => {
      rasterUrls[index] = publicUrl;
    });

    timings["rasterGeneration"] = Date.now() - rasterStart;
    console.log(
      `[Diagnostics] Raster generation: ${timings["rasterGeneration"]}ms (${
        Object.keys(rasterUrls).length
      }/${indices.length} uploaded)`,
    );
    ...
    for (const index of indices) {
      console.log(
        `[Diagnostics]   ${index}: ${
          timings[`${index}_total`] || "N/A"
        }ms (stats: ${timings[`${index}_stats`] || "N/A"}ms, first: ${
          timings[`${index}_firstImage`] || "N/A"
        }ms, last: ${timings[`${index}_lastImage`] || "N/A"}ms, map: ${
          timings[`${index}_mapTile`] || "N/A"
        }ms)`,
      );
    }
    if (timings["pixelSampling"]) {
      console.log(
        `[Diagnostics]   pixelSampling: ${
          timings["pixelSampling"]
        }ms (${cellData.length} points)`,
      );
    }
    if (timings["rasterGeneration"]) {
      console.log(
        `[Diagnostics]   rasterGeneration: ${timings["rasterGeneration"]}ms`,
      );
    }
    ...
    const metadata = {
      ...
      maxCloudCover,
      cloudCover: await evaluate(collection.aggregate_mean("cloud_cover"))
        .catch(() => null),
      resolution: "10m",
      indices,
      season,
