...
export async function initializeEarthEngine(): Promise<void> {
  ...
  if (!privateKey || !clientEmail) {
    throw new Error(
      "Earth Engine credentials (GOOGLE_PRIVATE_KEY or GOOGLE_CLIENT_EMAIL) missing from environment variables",
    );
  }
...
> {
  ...
  console.log(
    `🔍 Querying ${satellites.length} optical satellites for dates between ${startDate} and ${endDate}`,
  );
...
export function getSentinel1Collection(
  ...
  if (orbitPass !== "BOTH") {
    collection = collection.filter(
      ee.Filter.eq("orbitProperties_pass", orbitPass),
    );
  }
...
> {
  console.log(
    `🛰️  Fetching dates from all satellites (${startDate} to ${endDate})`,
  );
  ...
  const satelliteCounts = combined.reduce(
