...
export interface SpectralFeatures {
  ...
  ribinir?: number; // [−1, 1]  — RIBInir published blast index (Tian et al. 2023)
  ribired?: number; // [−1, 1]  — RIBIred red-edge variant / BLB proxy
...
/**
 * RIBInir anomaly (Tian et al. 2023). Blast lowers the index. Thresholds are
 * PROVISIONAL and the sign is UNCONFIRMED for the S2 band approximation
 * (ρ753→B7, ρ1102→B8A) — to be re-fit + sign-checked against diseased ground
 * truth. Onset re-centred below the observed healthy-paddy baseline (RIBInir
 * ≈ −0.03 over a clear kharif field) so healthy canopy reads ~0. Absent → 0.
 */
function ribinirAnomaly(ribinir: number | undefined): number {
  if (ribinir === undefined || Number.isNaN(ribinir)) return 0;
  return clamp01((-0.08 - ribinir) / 0.20); // healthy ≈ −0.03 → 0; full at −0.28
}

/**
 * RIBIred anomaly — red-edge variant of RIBI; interim BLB red-edge proxy until
 * RBBRI/RBBDI bands are extracted from ref [2]. PROVISIONAL threshold + UNCONFIRMED
 * sign. Onset re-centred below the observed healthy-paddy baseline (RIBIred ≈ −0.31
 * over a clear kharif field) so healthy canopy reads ~0.
 * TODO(RBBRI): replace with published RBBRI/RBBDI bands once available.
 */
function ribiredAnomaly(ribired: number | undefined): number {
  if (ribired === undefined || Number.isNaN(ribired)) return 0;
  return clamp01((-0.36 - ribired) / 0.20); // healthy ≈ −0.31 → 0; full at −0.56
}
 ...
 */
export function thermalConfounder(
  spec: SpectralFeatures,
): { mult: number; likely_abiotic: boolean } {
  const thermal = spec.thermal_stress;
  if (thermal === undefined || Number.isNaN(thermal)) {
    return { mult: 1, likely_abiotic: false };
  }
...
export function riceBlastRisk(
  ...
  const rawScore = spectralAnomaly * 0.42 +
    ndviDecline * 0.18 +
    moistureScore * 0.15 +
    weatherScore * 0.15 +
    redsiCheck * 0.05 +
  ...
  if (ribinirAnomaly(spec.ribinir) > 0.30) {
    signals.push("RIBInir blast index anomaly");
  }
...
export function sheathBlightRisk(
  ...
  const rawScore = heterogeneity * 0.35 +
    ndviDecline * 0.25 +
    wetScore * 0.25 +
    warmScore * 0.15;
...
export function blbRisk(
  ...
  // Interim BLB red-edge proxy: RIBIred + DWS (TODO: replace with RBBRI/RBBDI).
  ...
  const rawScore = waterSignal * 0.30 +
    redEdgeProxy * 0.10 +
    ndviDecline * 0.20 +
    wetScore * 0.25 +
    stormScore * 0.15;
...
export function downyMildewRisk(
  ...
  const rawScore = ndviDecline * 0.30 +
    cireSignal * 0.25 +
    weatherScore * 0.30 +
    moistureSignal * 0.15;
  ...
  if (weatherScore > 0.40) {
    signals.push("cool moist weather (downy mildew window)");
  }
...
export function leafSpotRisk(
  ...
  const rawScore = heterogeneity * 0.30 +
    ndviDecline * 0.25 +
    wetScore * 0.25 +
    dwsSignal * 0.15 +
    redsiCheck * 0.05;
...
export function parseGrowthStage(input: string | undefined): GrowthStage {
  ...
  if (lower.includes("seedling") || lower.includes("transplant")) {
    return "seedling";
  }
  if (lower.includes("tiller")) return "tillering";
  if (lower.includes("panicle") || lower.includes("pi")) {
    return "panicle_initiation";
  }
  if (
    lower.includes("head") || lower.includes("flower") || lower.includes("silk")
  ) return "heading";
  if (
    lower.includes("grain") || lower.includes("dough") ||
    lower.includes("dent") || lower.includes("milk")
  ) return "grain_fill";
