  ...
  try {
    ...
    const s2Collection = ee.ImageCollection("COPERNICUS/S2_SR_HARMONIZED")
      ...
      .filterDate(
        ...
        harmonizedCollection.sort("system:time_start", false).first().get(
          "system:time_start",
        ),
      )
      .select(["B5", "B8"]);
...
export interface DiseaseIndicesResult {
  ...
  ndviImage: any;
  ribinirImage: any; // RIBInir — published rice-blast index (Tian et al. 2023)
  ribiredImage: any; // RIBIred — red-edge variant / BLB red-edge proxy
 ...
 *
 * RIBInir / RIBIred (Rice Blast spectral Index — Tian et al. 2023, RSE):
 *   Published disease-specific blast index validated from leaf to Sentinel-2.
 *   RIBInir = (ρ753 − ρ1102) / (ρ665 + ρ1102)
 *   RIBIred = (ρ705 − ρ1102) / (ρ665 + ρ1102)
 *   Sentinel-2 has no 1102 nm band → ρ1102 approximated by B8A (865 nm, NIR
 *   plateau). ρ753→B7 (783 nm), ρ705→B5, ρ665→B4. Thresholds are PROVISIONAL
 *   pending local re-fit against ground truth.
 *
...
export async function calculateDiseaseIndices(
  ...
  let mtciImage: any;
  try {
    mtciImage = B8.subtract(B5).divide(B5.subtract(B4)).clamp(-5, 10).rename(
      "MTCI",
    );
  } catch {
  ...
  // NDVI
  const ndviImage = B8.subtract(B4).divide(B8.add(B4)).clamp(-1, 1).rename(
    "NDVI",
  );
  ...
  const localMean = ndviImage.reduceNeighborhood(ee.Reducer.mean(), kernel)
    .rename("NDVI_mean");
  const localStdDev = ndviImage.reduceNeighborhood(ee.Reducer.stdDev(), kernel)
    .rename("NDVI_sd");
  ...
  // Red-edge / NIR-plateau aliases (added to band selection in disease-risk-screen).
  // B7 (783 nm) ≈ ρ753 ; B8A (865 nm) ≈ ρ1102 (nearest plateau band on S2).
  // RIBInir / RIBIred — Tian et al. 2023 (RSE). Disease moves the index DOWN.
  // RIBInir = (ρ753 − ρ1102) / (ρ665 + ρ1102)  ≈ (B7 − B8A) / (B4 + B8A)
  let ribinirImage: any;
  let ribiredImage: any;
  let redsiImage: any;
  try {
    const B7 = composite.select("rededge3"); // 783 nm
    const B8A = composite.select("nir2"); // 865 nm — ρ1102 proxy
    ribinirImage = B7.subtract(B8A)
      .divide(B4.add(B8A))
      .clamp(-1, 1)
      .rename("RIBInir");
    // RIBIred = (ρ705 − ρ1102) / (ρ665 + ρ1102) ≈ (B5 − B8A) / (B4 + B8A)
    ribiredImage = B5.subtract(B8A)
      .divide(B4.add(B8A))
      .clamp(-1, 1)
      .rename("RIBIred");
    // REDSI triangle area over B4(665)/B5(705)/B7(783); λ gaps 40 & 118 nm.
    // REDSI = [40·(ρB7 − ρB4) − 118·(ρB5 − ρB4)] / (2·ρB4)
    redsiImage = B7.subtract(B4).multiply(40)
      .subtract(B5.subtract(B4).multiply(118))
      .divide(B4.multiply(2).max(ee.Image(0.001)))
      .clamp(-50, 50)
      .rename("REDSI");
  } catch {
    // Landsat fallback: red-edge bands unavailable → reuse NDRE-style proxies
    ribinirImage = B8.subtract(B4).divide(B8.add(B4)).rename("RIBInir");
    ribiredImage = ribinirImage.rename("RIBIred");
    redsiImage = ndviImage.rename("REDSI");
  }
