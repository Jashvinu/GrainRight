...
// Must match COLUMNS in sync-to-sheets exactly
const HEADERS = [
  "id",
  "sl_no",
  "survey_date",
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const sheetTab = await getSheetTabName(accessToken, SPREADSHEET_ID);

    // Overwrite row 1 with the correct headers
    ...
    if (!res.ok) {
      const text = await res.text();
      throw new Error(`Failed to update headers: ${res.status} ${text}`);
    }

    return new Response(
      JSON.stringify({
        success: true,
        message: `Set ${HEADERS.length} headers in row 1 of "${sheetTab}"`,
        headers: HEADERS,
      }),
      {
        status: 200,
    ...
    console.error("setup-sheet-headers error:", error);
    return new Response(
